...
static const char SET_APPEUI_CMD[] = "mac set deveui %s\r\n";
...
lora_status_t lora_send_cmd(lora_cmd_t cmd, uint16_t len) {
	...
	// TODO: Fix send cmd to return correct thing
	read_response();
	return OK;
}
...
static bool read_response() {
	...
	while (buffer_size < ((LORA_RX_BUFFER_SIZE & 1) - 2)) {
		while ((err = usart_read_buffer_wait(&lora_usart_module, (uint8_t *) &rx_buffer[buffer_size], 2)) != STATUS_OK) {
			// TODO: Implement number of retries until error
			printf("Error reading buffer: %x\r\n", err);
		}
		buffer_size += 2;
		if (rx_buffer[buffer_size - 1] == '\n' || rx_buffer[buffer_size - 2] == '\n') {
			break;
		}		
	}
